...
def clear_run_dir(out_dir: Path) -> None:
    ...
    for child in out_dir.iterdir():
        if child.is_file():
            child.unlink()
        elif child.name in {"ss_diff_debug", "mavf_v0_debug"}:
            for f in child.glob("*"):
                f.unlink()
...
def main() -> None:
    ...
    src_name = "chong"
    tar_name = "hudie"
    out_dir = ROOT / "outputs/analysis/mavf_v0_chong_hudie_m25/MAVF_V0_default"
    cache_dir = out_dir
    ...
    params = {
        ...
        "dump_mavf_v0_debug": True,
        "mavf_debug_step_idx": 24,
        "mavf_debug_block_idx": 23,
        "mavf_enable": True,
        "enable_cmf_v0": False,
    ...
    for morphing_idx in range(1, morphing_num - 1):
        ...
        for prefix in ("ss_sa_morphing", "slat_sa_morphing", "ss_ca_oc_morphing"):
            for f in cache_dir.glob(f"{prefix}{params['tfsa_cache_idx']}_*"):
                f.unlink()
        print(f"[MAVF-V0] frame {morphing_idx}/{morphing_num - 2} alpha={params['alpha']:.4f}")
    ...
    save_mp4(out_dir / "MAVF_V0_default_fixed_view.mp4", fixed_video, fps=12)
    print(f"[MAVF-V0] done: {out_dir}")
